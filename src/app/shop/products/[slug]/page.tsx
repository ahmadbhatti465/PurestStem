...
import { Star, ArrowLeft, Check, Shield, Truck, Leaf, ChevronRight } from "lucide-react";
...
export async function generateMetadata({
  ...
  if (!product) {
    return {
      title: "Product Not Found | PurestStem",
    };
  }
  ...
  return {
    title: `${product.name} | Buy Online in Pakistan | PurestStem`,
    description,
    keywords: [
      product.name,
      product.category.name,
      "herbal products Pakistan",
      `${product.category.name.toLowerCase()} Pakistan`,
      "buy online Pakistan",
      "cash on delivery",
    ],
    openGraph: {
    ...
    alternates: {
      canonical: `http://localhost:3000/shop/products/${product.slug}`,
    },
  };
}
...
export default async function ProductPage({
  ...
  const structuredData = {
    ...
    offers: {
      "@type": "Offer",
      url: `http://localhost:3000/shop/products/${product.slug}`,
      priceCurrency: "PKR",
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav aria-label="Breadcrumb">
        <ol className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <li>/</li>
          <li>
            <Link href="/shop/products" className="hover:text-green-700">Products</Link>
          </li>
          <li>/</li>
      ...
      <div className="grid md:grid-cols-2 gap-10 mb-16">
        <div className="relative aspect-square bg-gray-100 rounded-2xl overflow-hidden shadow-sm border border-gray-100">
          ...
          {discount > 0 && (
            <div className="absolute top-4 left-4 bg-red-500 text-white text-sm font-bold px-3 py-1.5 rounded-full shadow-sm">
              -{discount}%
            </div>
          )}
          {product.stock <= 5 && product.stock > 0 && (
            <div className="absolute top-4 right-4 bg-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="inline-flex items-center gap-2 bg-gray-50 text-gray-600 px-3 py-1.5 rounded-lg text-sm border border-gray-100">
      ...
      <div className="grid lg:grid-cols-3 gap-10 mb-16">
        ...
        <div className="bg-green-50 rounded-2xl p-6 border border-green-100">
          <h2 className="font-semibold text-gray-900 mb-4">Why You&apos;ll Love It</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {product.reviews.map((review) => (
              <div
                key={review.id}
                className="bg-white p-5 rounded-2xl border border-gray-100 hover:border-green-200 transition-all"
              >
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex">
                    {Array.from({ length: 5 }).map((_, i) => (
